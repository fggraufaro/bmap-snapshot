...
-- migration; it is never deployed from here. Query
-- pg_get_functiondef('geo.refresh_branch_competitors_10mi_v2'::regproc) (etc.)
-- any time to confirm what's actually live.
--
-- Applied via Supabase migrations "add_full_refresh_procedures_for_old_10mi_system"
-- and "fix_archive_10mi_radius_stats_dedup" (2026-09-24).
--
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_inserted FROM geo.branch_competitors_10mi_v2_new;
  IF v_inserted < v_total_branches * 5 THEN
    RAISE EXCEPTION 'branch_competitors_10mi_v2 rebuild produced only % rows for % branches - aborting before swap', v_inserted, v_total_branches;
  END IF;
  ...
  INSERT INTO geo.branch_competitors_10mi_v2 SELECT * FROM geo.branch_competitors_10mi_v2_new;

  GRANT SELECT ON geo.branch_competitors_10mi_v2 TO anon, authenticated, service_role;

  DROP TABLE geo.branch_competitors_10mi_v2_new;
  COMMIT;

  RAISE NOTICE 'geo.branch_competitors_10mi_v2 refreshed: % pairs for % branches (year %)', v_inserted, v_total_branches, v_year;
END;
$$;
